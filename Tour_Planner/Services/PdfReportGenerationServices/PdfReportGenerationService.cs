...
public class PdfReportGenerationService : IPdfReportGenerationService {
    ...
    public async Task GenerateOneTourReport(Tour tour, string path) {
        try {
            ...
            throw new UiLayerException($"Failed to create the PDF-Report for the specified path: {path}, and Tour with Name: {tour.Name} and ID: {tour.Id}!");
        }
    }
    ...
    public void GenerateToursSummaryReport(List<Tour> tours, string path) {
        try {
            ...
            throw new UiLayerException($"Failed to create the PDF-Report for the specified path {path} and the List of Tours!");
        }
    }
