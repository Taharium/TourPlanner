...
public class BusinessLogicOpenRouteService : IOpenRouteService
{
    ...
    public BusinessLogicOpenRouteService(IConfigOpenRouteService configOpenRouteService)
    {
        try {
            ...
            Logger.Error("Could not set base address for OpenRouteService: https://api.openrouteservice.org!");
...
//number formatter
            ...
            Logger.Error($"Could not get Geo-coordinates using the location {location}!");
            throw new BusinessLayerException($"Could not get Geo-coordinates using the location {location}! Please try again using different Locations!");
        }
    }
    ...
    public string GetDistance(JsonNode jsonNodeDirections)
    {
        try {
            ...
            Logger.Error($"Could not get the Distance using the JsonNode: {jsonNodeDirections}!");
    ...
    public string GetEstimatedTime(JsonNode jsonNodeDirections)
    {
        try {
            ...
            Logger.Error($"Could not get the estimated time using the JasonNode: {jsonNodeDirections}!");
    ...
    public async Task<List<string>> GetPlaces(string location)
    {
        try {
            ...
            Logger.Error($"Could not get the start- and/or end-location using the location {location}!");
            throw new BusinessLayerException($"Could not get the location using the location: {location}! Please try again using different Locations!");
        }
    }
    ...
    public async Task<JsonNode> GetRoute(string startLocation, string endLocation, TransportType transportType)
    {
        try {
            ...
            Logger.Error($"Could not generate the Route from {startLocation} to {endLocation}! Either Route is too long, or impossible to cross using the chosen transport type {transportType.ToString()}!");
            throw new BusinessLayerException($"Could not generate the Route from {startLocation} to {endLocation}! Either Route is too long, or impossible to cross using the chosen transport type {transportType.ToString()}! Please try again using different Locations!");
        }
